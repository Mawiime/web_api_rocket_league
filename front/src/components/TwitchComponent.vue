...
export default {
  ...
  methods : {
      ...
      retrievePics: async function(id){
        console.log("change");
        let fetchLink = `https://api.twitch.tv/helix/users?id=${id}`;
        ...
        for(let key in data.data){
          list.push({ida: id, image: data.data[key].profile_image_url});
        }
        return list;
      },

      fetchStreams : async function(){
        const response = await fetch('https://api.twitch.tv/helix/streams?game_id=30921',{
          method: 'get',
            headers: new Headers({
                'Authorization': process.env.VUE_APP_TWITCH_AUTHORIZATION,
                'Client-ID' : process.env.VUE_APP_TWITCH_CLIENT_ID
      ...
      retrieveUsefullData : async function(){
        let data = await this.fetchStreams();
        let dataFromGame = [];
        let listPics = [];
        for(let key in data.data){
          dataFromGame.push({
          ...
          listPics.push({id: dataFromGame[dataFromGame.length-1].id,
          name: dataFromGame[dataFromGame.length-1].user_name});
        }
  ...
  mounted : function () {
    let listTemp = this.retrieveUsefullData();
    this.retrieveAll(listTemp);
  },
}
</script>
